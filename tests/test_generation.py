"""Canonical generation protocol: catalog/compiler parity, diversity, schema and views."""
...
from icgs.data.collection.generation.camera import CAMERA_PROFILES, get_camera_profile
from icgs.data.collection.generation.compiler import compile_generation_catalog, events_from_steps
from icgs.data.collection.generation.batch import AttemptPlanner, provenance_from_plan
...
from icgs.data.collection.generation.layout import initialize_dataset_layout
from icgs.data.collection.generation.manifest import build_manifest
from icgs.data.collection.generation.perturbations import (
    SIMULATOR_CRASH,
    SUCCESS,
    VALID_FAILURE,
    apply_perturbation,
...
from icgs.data.collection.generation.report import observed_semantics_report, program_parity_report, training_eligible_ids
from icgs.data.collection.generation.steps import PROGRAM_FAMILIES, structured_steps
from icgs.data.datasets.generation_views import SUPPORTED_GENERATION_VIEWS, build_generation_view
...
from icgs.data.collection.generation.simulator_randomization import (
...
METHOD_PROFILE = Path("src/icgs/configuration/profiles/method.json")
GENERATION_PROFILE = Path("src/icgs/configuration/profiles/generation.json")
...
class ProtocolFreezeTests(unittest.TestCase):
    def test_generation_protocol_ids_are_frozen(self):
        self.assertEqual(GENERATION_PROTOCOL.dataset_version, "icgs-primary-v3")
        self.assertEqual(GENERATION_PROTOCOL.program_manifest_version, 3)
        self.assertEqual(GENERATION_PROTOCOL.episode_schema_version, "icgs_episode_v2")
        self.assertEqual(GENERATION_PROTOCOL.composition_protocol_id, "icgs-composition-primary-v2")
        self.assertEqual(GENERATION_PROTOCOL.controller_protocol_id, "rlbench-timed-ik-v2")
        self.assertEqual(GENERATION_PROTOCOL.minimum_execution_modes, 1)
        self.assertEqual(GENERATION_PROTOCOL.execution_modes, ("scripted_waypoint_v1",))
        self.assertEqual(GENERATION_PROTOCOL.phase, "1")
        self.assertEqual(GENERATION_PROTOCOL.train_programs, 20)
        self.assertEqual(GENERATION_PROTOCOL.development_programs, 4)
        self.assertEqual(GENERATION_PROTOCOL.test_programs, 12)
        self.assertEqual(GENERATION_PROTOCOL.pilot_program_ids, PILOT_IDS)
        self.assertEqual(GENERATION_PROTOCOL.views, ("D_geom", "D_temporal", "D_dyn", "D_task"))
        self.assertEqual(GENERATION_PROTOCOL.mixture_measured_on, "training_transitions")
        self.assertEqual(GENERATION_PROTOCOL.mixture_views, ("D_temporal", "D_dyn"))
        self.assertEqual(GENERATION_PROTOCOL.train_on, "train_core")
        self.assertEqual(GENERATION_PROTOCOL.validation, "train_val")
        self.assertEqual(GENERATION_PROTOCOL.train_nominal_successes_per_program, 200)
        self.assertEqual(GENERATION_PROTOCOL.train_perturbed_attempts_per_program, 80)
        self.assertEqual(GENERATION_PROTOCOL.eval_perturbed_attempts_per_program, 20)
        self.assertEqual(len(GENERATION_PROTOCOL.physics_passed_program_ids), 36)
        self.assertNotIn("D_recovery", GENERATION_PROTOCOL.views)
        self.assertNotIn("D_value", GENERATION_PROTOCOL.views)
        self.assertEqual(len(GENERATION_PROTOCOL.perturbation_kinds), 5)

    def test_canonical_manifest_is_the_only_generation_manifest(self):
        profile = json.loads(METHOD_PROFILE.read_text(encoding="utf-8"))
    ...
    def test_generation_profile_does_not_replace_canonical_defaults(self):
        payload = json.loads(GENERATION_PROFILE.read_text(encoding="utf-8"))
        ...
        canonical = json.loads(METHOD_PROFILE.read_text(encoding="utf-8"))
        self.assertNotEqual(canonical["config"]["dataset"]["schema_id"], "icgs_episode_v2")


class StructuredCatalogTests(unittest.TestCase):
    def test_structured_steps_match_catalog_text_for_every_generation_program(self):
        programs = [spec for spec in program_catalog().values() if spec.split in {"train", "development", "test"}]
    ...
    def test_required_train_families_are_present(self):
        ...
        self.assertTrue(set(GENERATION_PROTOCOL.required_train_families) <= train_families)
...
class CompilerParityTests(unittest.TestCase):
    ...
    def test_deferred_and_non_train_programs_are_not_training_eligible(self):
        eligible = training_eligible_ids(compile_generation_catalog())
        self.assertTrue(set(eligible) <= set(GENERATION_PROTOCOL.physics_passed_program_ids))
...
class CameraAndSchemaTests(unittest.TestCase):
    ...
    def test_held_out_provenance_is_materialized_for_eval_perturbation(self):
        ...
        validate_episode({**_episode_record(n_actions=2), "provenance": {
            **_episode_record(n_actions=2)["provenance"],
            **provenance,
    ...
    def test_episode_record_requires_t_plus_one_observations_and_dt(self):
        record = _episode_record(n_actions=3)
        validate_episode(record)
        record["online_observations"].pop()
        with self.assertRaisesRegex(ValueError, "T\\+1|observations"):
            validate_episode(record)

    def test_episode_record_rejects_v1_schema_id(self):
        record = _episode_record(n_actions=2)
...
class ViewTests(unittest.TestCase):
    def test_only_phase1_views_are_built(self):
        self.assertEqual(SUPPORTED_GENERATION_VIEWS, ("D_geom", "D_temporal", "D_dyn", "D_task"))
        records = [_episode_record(n_actions=4, episode_id="ep_a")]
        geom = build_generation_view(records, "D_geom")
        temporal = build_generation_view(records, "D_temporal")
        dyn = build_generation_view(records, "D_dyn")
        task = build_generation_view(records, "D_task")
        ...
        for name in ("D_value", "D_recovery", "D_pair"):
            with self.assertRaisesRegex(ValueError, "unsupported"):
                build_generation_view(records, name)

    def test_dyn_marks_external_intervention_only_at_timestep_scope(self):
        record = _episode_record(n_actions=4, episode_id="ep_pert")
        ...
        dyn = build_generation_view([record], "D_dyn", mix=False)
    ...
    def test_training_views_exclude_eval_and_train_val(self):
        ...
        val_record = _episode_record(n_actions=4, episode_id="ep_val")
        val_record["provenance"]["subset"] = "train_val"
        self.assertEqual(build_generation_view([val_record], "D_geom"), [])
        self.assertEqual(len(build_generation_view([val_record], "D_geom", role="validation")), 5)
...
class ReportAndLayoutTests(unittest.TestCase):
    def test_parity_report_lists_pilot_programs(self):
        ...
        compiled = compile_generation_catalog()
        record = _episode_record(n_actions=2, episode_id="ep_sem")
    ...
    def test_generation_manifest_contains_compiled_fields(self):
        manifest = build_manifest(APPROVED_MANIFEST)
        self.assertEqual(manifest["composition_protocol_id"], "icgs-composition-primary-v2")
        self.assertEqual(manifest["manifest_version"], 3)
        ...
        canonical = json.loads(APPROVED_MANIFEST.read_text(encoding="utf-8"))
        self.assertEqual(canonical["composition_protocol_id"], "icgs-composition-primary-v2")

    def test_phase1_layout_has_views_not_branches(self):
        import tempfile

        with tempfile.TemporaryDirectory() as directory:
            root = initialize_dataset_layout(directory)
...
def _episode_record(*, n_actions: int, episode_id: str = "ep_test") -> dict:
    import numpy as np
