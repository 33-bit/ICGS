...
from icgs.data.collection.v3.attempt_prep import plan_full_generation, prepare_attempt
from icgs.data.collection.v3.batch import (
    AttemptPlan,
    AttemptPlanner,
...
from icgs.data.collection.v3.quota import (
    QuotaCounts,
    next_episode_kind,
    quota_for_program,
    quota_met,
...
from icgs.data.collection.v3.report import generation_parity_gate, split_disjointness_report
from icgs.data.collection.v3.compiler import compile_v3_catalog
from icgs.data.collection.v3.episode_record import assemble_episode_v2, classify_generation_outcome, quarantine_sidecar
...
class BatchPlanTests(unittest.TestCase):
    def setUp(self):
        self.manifest = build_v3_manifest(V2_MANIFEST)
    ...
    def test_pilot_batch_covers_family_programs_with_nominal_and_perturbed(self):
        ...
        self.assertEqual(set(by_program), set(V3_PROTOCOL.pilot_program_ids))
    ...
    def test_assemble_episode_v2_from_attempt_plan(self):
        import numpy as np
    ...
    def test_inner_and_outer_magnitude_buckets_do_not_overlap(self):
        bound = V3_PROTOCOL.target_offset_m
        for index in range(40):
            ...
            self.assertLess(inner_r, V3_PROTOCOL.perturbation_inner_fraction)
            self.assertGreaterEqual(outer_r, V3_PROTOCOL.perturbation_inner_fraction)
    ...
    def test_v3_manifest_records_perturbation_bounds_and_position_bins(self):
    ...
    def test_v3_manifest_uses_wrist_depth_camera(self):
        self.assertEqual(self.rows["T01"]["randomization"]["camera_profile_id"], "rlbench-wrist-depth-v1")
        v2 = json.loads(V2_MANIFEST.read_text())
        v2_t01 = next(row for row in v2["catalog"] if row["program_id"] == "T01")
        self.assertEqual(v2_t01["randomization"]["camera_profile_id"], "rlbench-base-cameras-v1")
