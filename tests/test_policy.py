...
class ConstantSampler:
    ...
    def sample(self, network, data):
        from ip.types import ActionTrajectory
...
class PolicyTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertIsNotNone(importlib.util.find_spec('ip.policy'), 'Missing public policy')
    ...
    def policy(self, custom=True):
        ...
        from ip.algorithms.diffusion import OriginalDiffusionObjective
    ...
    def test_context_rejects_cross_policy_and_resets_cache(self):
        from ip.types import PreparedContext
        a, b = self.policy(), self.policy()
        context = PreparedContext([], a.context_owner, embeddings=torch.ones(1), positions=torch.ones(1))
