...
from ip.configs.original import from_legacy
...
def build_components(config, factories=None):
    ...
    from ip.models.backbone import original_stages
...
def build_policy(config, factories=None):
    from ip.algorithms.diffusion import OriginalDiffusionSampler, OriginalDiffusionObjective, original_scheduler
    from ip.policy import InstantPolicy
    config = from_legacy(config)
    f = factories or ComponentFactories()
...
def read_experiment_config(directory):
    ...
    from ip.configs.original import from_resolved
...
def load_policy(directory='./checkpoints', model_name='model.pt', *, mode='eval',
    ...
    from pathlib import Path
    from ip.configs.original import profile
