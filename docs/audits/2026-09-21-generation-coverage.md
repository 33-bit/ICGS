# ICGS generation plan/code coverage audit

Date: 2026-09-21  
Audit revision: working tree after coordinator/watchdog and raw-executor repair
Scope: historical distributed generation implementation and the canonical
[generation owner](../components/generation.md),
the generation contract, distributed runtime, pilot runner, materializer,
...
|---|---|---|
| Program semantics, layouts, predicates | `src/icgs/data/collection/generation/{steps,compiler,scene_layouts}.py` | Used by build and pilot; parity/build smoke exercised |
| Attempt planning and quotas | `generation/{batch,quota,diversity,perturbations}.py` | Unit-tested; centralized planner used by coordinator |
| Simulator execution | `scripts/generation_episode_worker.py` | RLBench smoke runner; now captures measured simulator observations rather than a repeated tip point |
| Closed episode/attempt archive | `generation/rlbench_attempt.py` and pilot writer | Full layout + artifact manifest required by validator |
| Queue and worker | `generation/distributed_queue.py`, `scripts/generation_worker.py` | Atomic claim/ready states; worker is credential-free |
| Validation/ingestion | `generation/distributed_validation.py`, coordinator | Identity, checksums, timeline, schema, layout and artifact manifest checks |
| Resume/planning | `generation/distributed_planner.py`, coordinator resume helpers | Reconstructs pending/claimed/ready/ingested/published state; remote manifest fetched on startup |
| HF publication | `generation/distributed_publication.py` | Semantic paths, compact manifest, receipts/views, bounded batches and remote hash verification hook |
| Launch/keepalive | `scripts/generation_launch.py`, watchdog | Exact 200 displays, detached output, atomic PID receipt, bounded replacement and coordinator lock |
| Training views | `src/icgs/data/datasets/generation_views.py` | Pointer generation is now emitted per episode; sampler remains downstream |
...
- Historical Hugging Face paths from the retired run can be removed only in a
  separately authorized remote cleanup; this source migration does not mutate
  remote data.
...
5. Publish one repaired preflight batch under the canonical `generation/`
   prefix and verify every remote hash.
6. Handle any historical remote batch only after a remote file inventory and
   separate cleanup authorization; this source migration does not mutate it.
...
2. `rho`, `nu`, and `epsilon` task-label arrays are now materialized from
   ...
   authorized program.
3. `GENERATION_PROTOCOL.layout_version == 3` identifies the generation dataset tree, while the
