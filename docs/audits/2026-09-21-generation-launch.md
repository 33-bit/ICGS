# Historical distributed generation launch record

The session names, protocol strings and Hugging Face paths below are preserved
verbatim as historical evidence. They are not current source/module names or
operator commands; use the canonical [generation owner](../components/generation.md).

Status: **BLOCKED BEFORE FULL LAUNCH**

This record is historical evidence only. It is not an operator instruction and
is updated only from fresh receipts produced by the committed
...
| parity | retired parity entry point | PASS: all programs matched |
| 36 task build | `generation_build_tasks.py --programs ...` | PASS: 36/36 built |
| 36-program strict smoke | `generation_episode_worker.py` | FAIL: 34 success, 2 valid_failure (`T03`, `T12`), 0 simulator crashes |
...
The fresh v6e1 VM passed parity and built all 36 task models. The strict
...
the protocol's `push_contact_offset_m = 0.045` and commands the contact point at
