...
| `primary_v2` | Không ghi đè. Manifest `approved_composition_manifest.json` và profile `method.json` giữ nguyên. |
...
- Protocol: `src/icgs/data/collection/generation/protocol.py`
- Profile: `src/icgs/configuration/profiles/generation.json`
- Manifest: `artifacts/composition/approved_composition_manifest.json`
- Runtime owner: `docs/components/generation.md`
...
Generation catalog có 36 program: 20 train, 4 development và 12 test. Program
exploratory cũ không thuộc catalog này.
...
- Stop condition: `quota_met` trong `src/icgs/data/collection/generation/quota.py`. Không stop vì `len(published_successes) == 280`.
...
Hàm kiểm tra: `split_disjointness_report` trong `src/icgs/data/collection/generation/report.py`.
...
Gate: `generation_parity_gate`
(`src/icgs/data/collection/generation/report.py`). Coordinator chỉ được scale
khi gate này pass. Program lệch → `blocked`, không scale.
...
`build_generation_view(..., role="validation"|"evaluation")` cho `train_val` và dev/test.
...
Generation planner ingest `scene_signature` từ manifest khi resume.
...
quota.next_episode_kind
  ...
  → assemble_episode  hoặc  assemble_attempt_record
```
...
| File | Việc |
|---|---|
| `src/icgs/data/collection/generation/quota.py` | Stop condition attempt |
| `src/icgs/data/collection/generation/batch.py` | AttemptPlanner, magnitude bucket, provenance |
| `src/icgs/data/collection/generation/diversity.py` | Seed, strata, signature, duplicate |
| `src/icgs/data/collection/generation/attempt_prep.py` | `prepare_attempt`, `plan_full_generation` |
| `src/icgs/data/collection/generation/perturbations.py` | 5 kind, scope |
| `src/icgs/data/collection/generation/report.py` | Parity, leakage, mixture |
| `src/icgs/data/datasets/generation_views.py` | Pointer views + 70/30 |
| `scripts/generation_episode_worker.py` | Apply layout; không remap push |
| `scripts/generation_worker.py` | Claim job và materialize closed result |
| `scripts/generation_coordinator.py` | Quota, validation, manifest và publication |
| `scripts/generation_launch.py` | Preflight và launch distributed run |

Manifest production phải authorize; không có compatibility protocol selector.
...
```bash
ICGS_GENERATION_ATTEMPT_JSON=/path/plan.json \
ICGS_GENERATION_WRITE_EPISODE=/path/output \
  python scripts/generation_episode_worker.py T01
```

Distributed launcher (sau preflight và smoke receipt riêng):

```bash
PYTHONPATH=src python scripts/generation_launch.py \
  --run-root /content/icgs-generation-run \
  --workers 200 \
  --approved-manifest artifacts/composition/approved_composition_manifest.json \
  --code-revision COMMIT_SHA \
  --smoke-receipt /content/generation-smoke.json \
...
```bash
PYTHONPATH=src python3 -B -m pytest -q tests/test_generation.py tests/test_generation_*.py
python3 -B scripts/validate_fast.py
```

Fresh validation evidence belongs in the canonicalization plan and must not be
inferred from historical counts in this contract.
...
- Production has no legacy protocol-selection or `--allow-planned` path.
...
| Có compatibility protocol selector hoặc `--allow-planned`? | Không. |
