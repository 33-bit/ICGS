...
Colab launchers, and ad-hoc recovery and publication scripts. Before this
migration, the distributed worker still depended on compatibility-collector
execution helpers, so the live behavior had to move before the legacy file could
be removed.
...
- the compatibility collector after its execution/materialization behavior is
  moved into the canonical package;
- exploratory generation launchers that are not part of the generation dataset;
...
| legacy version-coded worker | `generation_worker.py` |
| legacy version-coded coordinator | `generation_coordinator.py` |
| legacy version-coded watchdog | `generation_watchdog.py` |
| legacy version-coded launcher | `generation_launch.py` |
| legacy pilot episode worker | `generation_episode_worker.py` |
| version-coded composition manifest | `approved_composition_manifest.json` |
