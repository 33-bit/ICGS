...
Construct MethodConfig once at the outer entry point, validate operation readiness, pass only relevant typed sections into each component, and save resolved_config() before a job. Add nondefault propagation tests at each implemented seam so accepted-but-unused options are not silently called configurable.
