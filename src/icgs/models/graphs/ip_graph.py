...
import torch
from icgs.models.layers.embeddings import PositionalEncoder
from icgs.models.layers.embeddings import SinusoidalPosEmb
from icgs.configuration.schema import GraphConfig


class GraphRep(nn.Module):
    def __init__(self, config: GraphConfig, batch_size=1, device='cpu'):
        super(GraphRep, self).__init__()
        if isinstance(config, dict):  # Historical public constructor, without sharing the mapping.
            batch_size, device = config['batch_size'], config['device']
            config = GraphConfig(num_demos=config['num_demos'], traj_horizon=config['traj_horizon'],
                                 num_scene_nodes=config['num_scenes_nodes'], local_num_freq=config['local_num_freq'],
                                 embd_dim=config['local_nn_dim'], pred_horizon=config['pre_horizon'],
                                 pos_in_nodes=config['pos_in_nodes'])
        ################################################################################################################
        # Store the parameters.
        self.batch_size = batch_size
        self.num_demos = config.num_demos
        self.traj_horizon = config.traj_horizon
        self.num_scenes_nodes = config.num_scene_nodes
