...
def bounds_from_row(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "camera_profile_id": randomization.get("camera_profile_id", GENERATION_PROTOCOL.camera_profile_id),
        "lighting_profile_id": randomization.get("lighting_profile_id", GENERATION_PROTOCOL.lighting_profile_id),
    }
...
def sample_intervention(
    ...
    *,
    ...
    if kind == "action_pose_offset":
        params = {
            "dx_m": round(_bucket_magnitude(signed, GENERATION_PROTOCOL.target_offset_m, bucket), 4),
            "dy_m": round(_bucket_magnitude(signed_y, GENERATION_PROTOCOL.target_offset_m, bucket), 4),
            "d_yaw_deg": round(_bucket_magnitude(signed, GENERATION_PROTOCOL.target_rotation_offset_deg, bucket), 3),
        }
    elif kind == "gripper_timing":
        params = {"delta_intervals": -1 if seed % 2 == 0 else 1}
    elif kind == "object_displacement":
        params = {
            "dx_m": round(_bucket_magnitude(signed, GENERATION_PROTOCOL.object_shift_m, bucket), 4),
            "dy_m": 0.0,
...
def _bucket_magnitude(signed: float, bound: float, bucket: str) -> float:
    ...
    inner = GENERATION_PROTOCOL.perturbation_inner_fraction
...
class AttemptPlanner:
    ...
    def __init__(
        ...
        *,
        ...
        self.spec = get_generation_program(program_id)
        self.collection_seed = GENERATION_PROTOCOL.collection_seed if collection_seed is None else collection_seed
        self.bounds = bounds
    ...
    def _build_plan(
        ...
        plan = AttemptPlan(
            ...
            episode_id=f"episode-{self.program_id.lower()}-{self.index:05d}",
...
def plan_program_attempts(
    ...
    *,
    ...
    n_nominal = GENERATION_PROTOCOL.train_nominal_successes_per_program if n_nominal is None else n_nominal
    n_perturbed = GENERATION_PROTOCOL.train_perturbed_attempts_per_program if n_perturbed is None else n_perturbed
    if planner is None:
...
def plan_pilot_batch(rows_by_id: Mapping[str, Mapping[str, Any]]) -> list[AttemptPlan]:
    ...
    for program_id in GENERATION_PROTOCOL.pilot_program_ids:
        row = rows_by_id[program_id]
        plans.extend(
            plan_program_attempts(
                program_id,
                n_nominal=GENERATION_PROTOCOL.pilot_nominal_per_program,
                n_perturbed=GENERATION_PROTOCOL.pilot_perturbed_per_program,
...
def provenance_from_plan(plan: AttemptPlan, *, binding: Mapping[str, Any]) -> dict[str, Any]:
    ...
    provenance = {
        ...
        "program_id": plan.program_id,
        "program_semantics_version": GENERATION_PROTOCOL.program_semantics_version,
        "split": split,
        ...
        "calibration_id": GENERATION_PROTOCOL.camera_profile_id,
        "camera_intrinsics_id": f"{GENERATION_PROTOCOL.camera_profile_id}-intrinsics",
        "camera_extrinsics_id": f"{GENERATION_PROTOCOL.camera_profile_id}-extrinsics",
        "action_space_id": GENERATION_PROTOCOL.action_space_id,
        "orientation_convention": GENERATION_PROTOCOL.orientation_convention,
        "gripper_unit": GENERATION_PROTOCOL.gripper_unit,
        ...
        "controller_version": GENERATION_PROTOCOL.controller_protocol_id,
        "simulator_version": GENERATION_PROTOCOL.simulator_version,
        "physics_engine_version": GENERATION_PROTOCOL.physics_engine_version,
    ...
    for field in GENERATION_PROTOCOL.phase2_reserved_fields:
