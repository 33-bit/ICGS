"""Compile generation programs from structured catalog steps, not hardcoded routines."""
...
from icgs.data.collection.generation.steps import ProgramStep, get_generation_program
...
def compile_program(program_id: str, *, predicate_tolerance_m: float = 0.01) -> CompiledTask:
...
def compile_generation_catalog(manifest_path: str | Path | None = None) -> dict[str, CompiledTask]:
    from icgs.data.collection.generation.steps import GENERATION_PROGRAMS

    tolerances: dict[str, float] = {}
    if manifest_path is not None:
        data = json.loads(Path(manifest_path).read_text(encoding="utf-8"))
        if data.get("composition_protocol_id") not in {None, GENERATION_PROTOCOL.composition_protocol_id}:
            raise ValueError("generation compiler requires the canonical composition protocol")
