...
def mix_training_transitions(
    ...
    *,
    ...
    if view not in GENERATION_PROTOCOL.mixture_views:
        return rows
    ...
    if not perturbed or not nominal:
        return rows
    nom_frac, pert_frac = GENERATION_PROTOCOL.warmup_mixture
    rng_seed = GENERATION_PROTOCOL.collection_seed if seed is None else seed
