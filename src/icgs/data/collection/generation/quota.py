...
def quota_for_program(program_id: str) -> CollectionQuota:
    ...
    if spec.split == "train":
        nominal = GENERATION_PROTOCOL.train_nominal_successes_per_program
        perturbed = GENERATION_PROTOCOL.train_perturbed_attempts_per_program
    else:
        nominal = GENERATION_PROTOCOL.eval_contexts_per_composition
        perturbed = GENERATION_PROTOCOL.eval_perturbed_attempts_per_program
    return CollectionQuota(
        program_id=program_id,
        split=spec.split,
        nominal_success_target=nominal,
        perturbed_attempt_target=perturbed,
        max_nominal_attempts=nominal * GENERATION_PROTOCOL.quota_attempt_cap_multiplier,
        max_perturbed_attempts=max(perturbed, 1) * GENERATION_PROTOCOL.quota_attempt_cap_multiplier if perturbed else 0,
    )
...
def valid_attempt_mixture(counts: QuotaCounts) -> dict[str, Any]:
    ...
    return {
        ...
        "training_mixture_target": list(GENERATION_PROTOCOL.warmup_mixture),
        "training_mixture_measured_on": GENERATION_PROTOCOL.mixture_measured_on,
        "training_mixture_views": list(GENERATION_PROTOCOL.mixture_views),
    }
