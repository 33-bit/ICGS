...
from icgs.data.collection.generation.compiler import CompiledTask
from icgs.data.collection.generation.diversity import nearest_signature_distance, scene_signature
from icgs.data.collection.generation.perturbations import INVALID_OBSERVATION, SIMULATOR_CRASH
from icgs.data.collection.generation.protocol import GENERATION_PROTOCOL
from icgs.data.collection.generation.quota import valid_attempt_mixture, QuotaCounts, record_outcome
from icgs.data.collection.generation.steps import structured_steps
from icgs.data.schemas.episode_records import validate_episode


def generation_parity_gate(
    compiled: Mapping[str, CompiledTask],
...
def training_eligible_ids(compiled: Mapping[str, CompiledTask]) -> tuple[str, ...]:
    return tuple(pid for pid, spec in compiled.items() if spec.training_eligible)


def program_parity_report(compiled: Mapping[str, CompiledTask]) -> dict[str, Any]:
...
def perturbation_report(records: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "allowed_kinds": list(GENERATION_PROTOCOL.perturbation_kinds),
    }
