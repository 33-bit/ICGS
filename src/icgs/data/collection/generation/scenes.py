"""Scene layouts for generation compilation. Geometry only; events come from structured steps."""
...
def load_scene_layouts() -> dict[str, Any]:
    if not _LAYOUT_PATH.is_file():
        raise FileNotFoundError(f"generation scene layouts missing: {_LAYOUT_PATH}")
    data = json.loads(_LAYOUT_PATH.read_text(encoding="utf-8"))
    if not isinstance(data, dict) or not data:
        raise ValueError("generation scene layouts must be a non-empty object")
    return data
...
def get_scene_layout(program_id: str) -> dict[str, Any]:
    ...
    if program_id not in layouts:
        raise KeyError(f"no generation scene layout for {program_id}")
