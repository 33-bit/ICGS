...
from icgs.data.collection.v3.lighting import get_lighting_profile, lighting_profile_ids
...
def episode_seed(
    ...
    *,
    ...
    version = dataset_version or V3_PROTOCOL.dataset_version
...
def sample_randomization(
    ...
    *,
    ...
    n_pos = V3_PROTOCOL.position_strata
    n_yaw = V3_PROTOCOL.yaw_strata
    n_scale = V3_PROTOCOL.scale_strata
    reserved = min(V3_PROTOCOL.eval_held_out_position_bin, n_pos - 1)
    ...
    gap = V3_PROTOCOL.inter_object_gap_m
    wp = V3_PROTOCOL.waypoint_delta_m
    ...
    sample = {
        ...
        "camera_profile_id": bounds.get("camera_profile_id", V3_PROTOCOL.camera_profile_id),
...
def train_subset_for_sample(sample: Mapping[str, Any], *, core_fraction: float | None = None) -> str:
    fraction = V3_PROTOCOL.train_core_fraction if core_fraction is None else core_fraction
...
def scene_signature(sample: Mapping[str, Any]) -> str:
    ...
    payload = {
        ...
        "tx": _quantize(float(translation.get("x", 0.0)), V3_PROTOCOL.pose_duplicate_threshold_m),
        "ty": _quantize(float(translation.get("y", 0.0)), V3_PROTOCOL.pose_duplicate_threshold_m),
        "tz": _quantize(float(translation.get("z", 0.0)), V3_PROTOCOL.pose_duplicate_threshold_m),
        ...
        "gap_x": _quantize(float(gap.get("dx_m", 0.0)), V3_PROTOCOL.pose_duplicate_threshold_m),
        "gap_y": _quantize(float(gap.get("dy_m", 0.0)), V3_PROTOCOL.pose_duplicate_threshold_m),
        "approach_x": _quantize(float(approach.get("dx_m", 0.0)), V3_PROTOCOL.pose_duplicate_threshold_m),
        "approach_y": _quantize(float(approach.get("dy_m", 0.0)), V3_PROTOCOL.pose_duplicate_threshold_m),
...
def is_duplicate_episode(
    ...
    *,
    ...
    if known:
        return True
    if previous:
        threshold = V3_PROTOCOL.pose_duplicate_threshold_m
        same_asset = [
