...
from icgs.data.collection.v3.perturbations import apply_perturbation
...
def plan_full_generation(
    ...
    *,
    ...
    seed = V3_PROTOCOL.collection_seed if collection_seed is None else collection_seed
    for program_id, row in rows_by_id.items():
        spec = get_v3_program(program_id)
        from icgs.data.collection.v3.batch import bounds_from_row

        bounds = bounds_from_row(row)
        family = row.get("asset_family_id")
        n_nominal = (
            V3_PROTOCOL.train_nominal_successes_per_program
            if spec.split == "train"
            else V3_PROTOCOL.eval_contexts_per_composition
        )
        n_perturbed = (
        ...
        planner = AttemptPlanner(
            program_id,
            collection_seed=seed if spec.split == "train" else seed + V3_PROTOCOL.eval_perturbation_seed_offset,
            bounds=bounds,
