"""Structured v3 program steps. Catalog text in programs.py remains the string form."""
...
def _prog(
    ...
    physics_ok = program_id in V3_PROTOCOL.physics_passed_program_ids
...
V3_PROGRAMS: Mapping[str, V3ProgramSpec] = MappingProxyType({spec.program_id: spec for spec in _PROGRAMS})
...
def structured_steps(program_id: str) -> tuple[ProgramStep, ...]:
    if program_id not in V3_PROGRAMS:
        raise ValueError(f"unknown v3 program_id: {program_id!r}")
    return V3_PROGRAMS[program_id].steps


def get_v3_program(program_id: str) -> V3ProgramSpec:
    if program_id not in V3_PROGRAMS:
        raise ValueError(f"unknown v3 program_id: {program_id!r}")
    return V3_PROGRAMS[program_id]
...
def assert_catalog_coverage() -> None:
    ...
    actual = set(V3_PROGRAMS)
    if expected != actual:
        raise RuntimeError(f"v3 catalog coverage mismatch missing={sorted(expected-actual)} extra={sorted(actual-expected)}")
