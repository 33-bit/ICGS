...
SUPPORTED_V3_VIEWS = V3_PROTOCOL.views
...
def _include_record(provenance: Mapping[str, Any], role: str) -> bool:
    ...
    if role == "train":
        if split in _HELD_OUT_SPLITS:
            return False
        if subset == V3_PROTOCOL.validation:
            return False
        return True
    if role == "validation":
        return split == "train" and subset == V3_PROTOCOL.validation
    if role == "evaluation":
...
def build_v3_view(
    ...
    *,
    ...
    if view not in SUPPORTED_V3_VIEWS:
        raise ValueError(f"unsupported phase-1 view: {view}")
    samples: list[dict[str, Any]] = []
    apply_mix = V3_PROTOCOL.mixture_measured_on == "training_transitions" if mix is None else mix
